...
def myNetwork():
    ...
    info('* Ingrese cantidad de host por sucursal (menor a 210) \n')
    try:
       hosts= int(raw_input('Host: '))
    except ValueError:
       sys.exit("No es un numero")
    if sucursales >210:
       sys.exit("No es menor a 32")

    print("Creando red para %s sucursales con %s hosts por sucursal" %(sucursales,hosts))
	...
	for i in range(hosts):
		i2=i+1
		G["h%i_%s"%(n2,i2)]= net.addHost('h%s_%s'%(n2,i2), cls=Host, ip='10.0.%s.%s/24'%(n2,h), defaultRoute=None)
		h=h-1
	...
	for i in range(hosts):
		i2=i+1
		net.addLink(G["h%i_%s"%(n2,i2)],G["s%i_lan"%n2])
	...
	for i in range(hosts):
		i2=i+1
		net['h%s_%s'%(n2,i2)].cmd('ip route add 0/0 via 10.0.%s.1'%n2)
